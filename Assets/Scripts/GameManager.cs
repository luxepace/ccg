...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        Debug.Log("[GameManager] Start() вызван. Сцена чистая!");
    ...
    {
        // Атакующая карта наносит урон защищающейся карте
        defender.Card.GetDamage(attacker.Card.Attack);
        attacker.OnDamageDeal();
        defender.OnTakeDamage(attacker);

        // Защищающаяся карта наносит контратаку атакующей карте
        attacker.Card.GetDamage(defender.Card.Attack);
        attacker.OnTakeDamage();

        // Проверка состояния жизни обеих карт
